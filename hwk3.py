...
if __name__ == "__main__":
    ...
    for x_val in np.arange(-5, 5.01, h):
        ...
        cnt_diff = central_difference(x_val, h, f)
        ...
        #solving for error
        centralError = relative_error(analytic_diff, cnt_diff)
        forwardError = relative_error(analytic_diff, fw_diff)
        #storing error as tuple
        errorData = (forwardError, centralError)
        #appending error to list
        error_data_range.append(errorData)
    ...
    np.savetxt(fname_part2, np.array(error_data_range), delimiter=',',header=('fwd error' + 'cnt error'), fmt = '%.05f')
